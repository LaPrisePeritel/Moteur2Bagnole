...
class A4ENGINE_API ResourceManager
{
	public:
		...
		const std::shared_ptr<AudioClip>& GetAudioClip(const char* audioPath);
	...
	private:
		...
		std::shared_ptr<AudioClip> m_missingAudioClip;
		...
		std::unordered_map<std::string /*audioPath*/, std::shared_ptr<AudioClip>> m_audioClips;
